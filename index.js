...
function renderSVG() {
  if (k8sStatus) {
    ...
    if (k8sStatus.IsK8sSystemHealthy) {
      badges.set(
        (0, badge_1.renderBadge)(k8sStatusText, "Healthy", healthyColor),
        "k8sStatus"
      );
    } else {
      badges.set(
        (0, badge_1.renderBadge)(k8sStatusText, "Unhealthy", unhealthyColor),
        "k8sStatus"
      );
    }
    ...
    if (k8sStatus.HealthyPodCount === allPodCount) {
      badges.set(
        (0, badge_1.renderBadge)(
          podStatusText,
          `${k8sStatus.HealthyPodCount}/${allPodCount}`,
          healthyColor
        ),
        "podStatus"
      );
    ...
    const allNodeCount =
      k8sStatus.HealthyNodeCount + k8sStatus.UnhealthyNodeCount;
    if (k8sStatus.HealthyNodeCount === allNodeCount) {
      badges.set(
        (0, badge_1.renderBadge)(
          nodeStatusText,
          `${k8sStatus.HealthyNodeCount}/${allNodeCount}`,
          healthyColor
        ),
        "nodeStatus"
      );
...
async function main() {
  ...
  console.log("Done.");
  core.setOutput("k8sStatus-SVG-Path", `${__dirname}/public/k8sStatus.svg`);
